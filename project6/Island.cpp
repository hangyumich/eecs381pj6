...
Island::Island(std::istream& is): Sim_object(is){
    position = read_point(is);
    fuel = read_double(is);
    production_rate = read_double(is);
}
...
void Island::save(std::ostream & os) const {
    Sim_object::save(os);
    os << position << " "  << fuel << " " << production_rate << endl;
}